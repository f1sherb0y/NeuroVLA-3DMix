...
Clean-room rename/clone of Qwen_GR00T for world-model-backbone VLA:
    - Uses WorldModelVLMInterface (not a Qwen VLM) as visual encoder
    - Reads config.framework.world_model.* (not config.framework.qwenvl.*)
    - Attribute renamed: self.qwen_vl_interface -> self.world_model_encoder
    - Framework-local wrapper: prepare_inputs (still calls interface.build_vlm_inputs
      under the hood; interface method rename deferred to Phase 3+)
...
from AlphaBrain.model.modules.world_model import get_world_model_encoder
...
@FRAMEWORK_REGISTRY.register("WorldModelVLA")
class WorldModelVLA(BaseFramework):
    """
    World-Model-backbone vision-language-action model.

    Components:
      - WorldModelVLMInterface: Cosmos-Predict2 / V-JEPA2 / WAN2 visual encoder
        with lightweight text encoder and cross-attention fusion
      - FlowmatchingActionHead (GR00T-N1.5) for future action sequence modeling

    Focus: Predict future continuous actions conditioned on images + instruction,
    optionally with next-frame video loss for joint WM + action training.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_yaml", type=str, default="./configs/models/world_model_vla_cosmos2.yaml", help="Path to YAML config")
    args, extra_cli_args = parser.parse_known_args()

    cfg = OmegaConf.load(args.config_yaml)
    model: WorldModelVLA = WorldModelVLA(cfg)
    print(model)
