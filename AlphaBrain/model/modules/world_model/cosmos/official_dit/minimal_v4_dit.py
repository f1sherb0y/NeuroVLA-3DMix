...
# KV cache stubs (not used in standard training/inference)
import torch.nn as nn


class KVCacheConfig:
    """Stub for KVCacheConfig."""
    pass


class AttentionOpWithKVCache(nn.Module):
    """Stub for AttentionOpWithKVCache."""
    pass
...
# Context parallel stub (not used in single-GPU mode)
def split_inputs_cp(x, seq_dim, cp_group):
    """Stub: returns input unchanged (no context parallelism in single-GPU mode)."""
    return x
from .conditioner import DataType
# NeighborhoodAttention stub
class NeighborhoodAttention:
    """Stub for NeighborhoodAttention (requires NATTEN library)."""
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            'NeighborhoodAttention requires the NATTEN library.'
        )
from .a2a_cp import MinimalA2AAttnOp, NattenA2AAttnOp
# WeightTrainingStat stub
class WeightTrainingStat(nn.Module):
    """Stub for WeightTrainingStat - base class that provides weight statistics logging.
    In the original code this is just an nn.Module with extra logging methods.
    Using plain nn.Module preserves all numerical behavior."""
    pass
# SACConfig stub
from dataclasses import dataclass as _sac_dataclass


@_sac_dataclass
class _SACConfig:
    """Stub for the base SACConfig from selective_activation_checkpoint."""
    mode: str = "none"
...
def predict2_2B_720_context_fn():
    ...
    def policy_fn(ctx, func, *args, **kwargs):
        ...
        if func == torch.ops.aten.mm.default:
            ...
            # (log import removed - using module-level logging)
