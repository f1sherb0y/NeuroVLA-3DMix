...
def _safe_read_parquet(path, **kwargs):
    """Read parquet compatible with pandas 2.x + pyarrow 14.

    fixed_size_list columns cause 'Wrong number of dimensions' in pandas 2.x.
    We read via pyarrow and convert list columns to Python objects manually.
    """
    table = _pq.read_table(str(path), **kwargs)
...
from AlphaBrain.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from AlphaBrain.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from AlphaBrain.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from AlphaBrain.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        self._image_type_video_keys = set()
        for new_key in le_modality_meta.video:
            ...
            # Handle image dtype (PNG bytes in parquet) vs video dtype (mp4 files)
            feature_dtype = le_video_meta.get("dtype", "video")
            if feature_dtype == "image":
                # Image data stored as bytes in parquet, no video fps info
                try:
                    channels = le_video_meta["shape"][le_video_meta["names"].index("channel")]
                except ValueError:
                    channels = le_video_meta["shape"][le_video_meta["names"].index("channels")]
                fps = le_info.get("fps", 10)  # Use dataset-level fps
                self._image_type_video_keys.add(new_key)
            else:
        ...
        # _image_type_video_keys is populated during the video metadata loop above
            ...
            for mode in ["delta", "rel"]:
                if mode not in le_statistics_by_mode:
                    try:
                        ...
                        print(
                            f"Warning: Failed to compute `{mode}` stats for {self.dataset_name}: {exc}. "
                            "Falling back to available action statistics."
                        )
    ...
    def _filter_trajectories_per_task(
        self,
        ...
        for ep in episode_metadata:
            ...
            task_name = ep["tasks"][0] if isinstance(ep["tasks"], list) else ep["tasks"]
            ep_to_task[ep_idx] = task_name
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self._lerobot_version == "v2.0":
            ...
            for episode in episode_metadata:
                ...
                # If filter_column is set, compute effective length
                if self._filter_column:
                    ...
                    if parquet_path.exists():
                        ep_data = _safe_read_parquet(parquet_path)
                        if self._filter_column in ep_data.columns:
                            mask = ~ep_data[self._filter_column].astype(bool)
                            self._traj_row_masks[ep_id] = mask.values
                            ep_len = int(mask.sum())
                trajectory_ids.append(ep_id)
                trajectory_lengths.append(ep_len)
            return self._filter_trajectories_per_task(
                np.array(trajectory_ids), np.array(trajectory_lengths), episode_metadata
            )
        # v3.0
            ...
            for file_path in file_paths:
                episodes_data = _safe_read_parquet(file_path)
                timestamp_cols = [
                    c
    ...
    def _get_tasks(self) -> pd.DataFrame:
        ...
        if self._lerobot_version == "v2.0":
            ...
            df = _safe_read_parquet(tasks_path)
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        # V2: store current step info for next_frame loading in _pack_sample
        self._v2_current_step = (trajectory_id, base_index)

        # V2: pre-compute next frame index so get_video can batch-read both frames
        self._v2_next_frame_index = None
        self._v2_cached_next_frame = None
        if (hasattr(self, "data_cfg") and self.data_cfg is not None
                and self.data_cfg.get("load_next_frame", False)):
            traj_idx_arr = np.where(self.trajectory_ids == trajectory_id)[0]
            traj_len = int(self.trajectory_lengths[traj_idx_arr[0]]) if len(traj_idx_arr) > 0 else 0
            next_idx = base_index + 1
            if next_idx < traj_len:
                self._v2_next_frame_index = next_idx
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        # --- V2: use cached next frame (batch-read in get_video, single file open) ---
        if (hasattr(self, "data_cfg") and self.data_cfg is not None
                and self.data_cfg.get("load_next_frame", False)):
            cached = getattr(self, "_v2_cached_next_frame", None)
            if cached is not None and len(cached) > 0:
                try:
                    next_img = cached[0]  # [H, W, C] numpy
                    sample["next_image"] = Image.fromarray(next_img).resize((224, 224))
                    sample["is_last_frame"] = False
                except Exception:
                    sample["next_image"] = None
                    sample["is_last_frame"] = True
                finally:
                    self._v2_cached_next_frame = None
            else:
                sample["next_image"] = None
    ...
    def _read_images_from_parquet(self, trajectory_id, key, step_indices):
        """Read image bytes from parquet for image-type data."""
        import io
        traj_data = self.curr_traj_data
        original_key = self.lerobot_modality_meta.video[key].original_key
        if original_key is None:
            original_key = key
        frames = []
        for idx in step_indices:
            img_data = traj_data.iloc[idx][original_key]
...
class LeRobotMixtureDataset(Dataset):
    ...
    def _sample_step_random(self):
        """Sample a random step without affecting sequential state (_step_pos).

        Used for retries to avoid corrupting the sequential sampling order.
        """
        rng = np.random.default_rng(random.randint(0, 2**31))
        dataset_index = rng.choice(len(self.datasets), p=self.dataset_sampling_weights)
        dataset = self.datasets[dataset_index]
        single_step_index = rng.choice(len(dataset.all_steps))
        trajectory_id, step = dataset.all_steps[single_step_index]
        return dataset, trajectory_id, step
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                if attempt == 0:
                    # First attempt: use the proper sequential sampling
                    dataset, trajectory_id, step = self.sample_step(index)
                else:
                    # Retries: sample randomly WITHOUT touching _step_pos
                    dataset, trajectory_id, step = self._sample_step_random()

                # Check video exists (skip for image-type data stored in parquet)
                key = dataset.modality_keys["video"][0].replace("video.", "")
                if key not in dataset._image_type_video_keys:
                    video_path = dataset.get_video_path(trajectory_id, key)
                    if not os.path.exists(video_path):
                        print(f"Video not found: {video_path}, retrying ({attempt + 1}/{max_retries})...")
                        continue

                # V2: pre-compute next frame index for batch-read in get_video
                dataset._v2_current_step = (trajectory_id, step)
                dataset._v2_next_frame_index = None
                dataset._v2_cached_next_frame = None
                if (hasattr(dataset, "data_cfg") and dataset.data_cfg is not None
                        and dataset.data_cfg.get("load_next_frame", False)):
                    import numpy as _np
                    traj_idx_arr = _np.where(dataset.trajectory_ids == trajectory_id)[0]
                    traj_len = int(dataset.trajectory_lengths[traj_idx_arr[0]]) if len(traj_idx_arr) > 0 else 0
                    if step + 1 < traj_len:
                        dataset._v2_next_frame_index = step + 1
    ...
    def _detect_action_norm_mode(self) -> str | None:
        """Detect the dominant action normalization mode from dataset transforms.

        Iterates through the first dataset's composed transforms to find
        StateActionTransform instances that apply to action keys, then returns
        the most common non-binary normalization mode (e.g. 'q99' or 'min_max').
        Returns None if no action normalization mode is found.
        """
        from AlphaBrain.dataloader.gr00t_lerobot.transform.state_action import StateActionTransform
        for dataset in self.datasets:
            ...
            for t in transform_list:
                if isinstance(t, StateActionTransform) and hasattr(t, 'normalization_modes'):
                    action_modes = [
                        mode for key, mode in t.normalization_modes.items()
                        if key.startswith("action.") and mode != "binary"
                    ]
                    if action_modes:
                        detected = action_modes[0]
                        print(f"[save_dataset_statistics] Detected action norm_mode: '{detected}' "
                              f"(from {len(action_modes)} action keys)")
                        return detected
        print("[save_dataset_statistics] WARNING: Could not detect action norm_mode from transforms, "
              "defaulting to None. Eval may use incorrect unnormalization!")
        return None
