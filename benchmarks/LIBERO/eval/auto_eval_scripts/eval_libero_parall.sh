...
export LIBERO_HOME="${LIBERO_HOME:-/path/to/LIBERO}"  # Root directory of the LIBERO project
export LIBERO_python="${LIBERO_python:-/path/to/envs/lerobot/bin/python}"  # Path to the Python environment
export alphabrain_python="${alphabrain_python:-/path/to/envs/AlphaBrain/bin/python}"  # Path to the Python environment

# === End of environment variable configuration ===
export LIBERO_CONFIG_PATH=${LIBERO_HOME}/libero  # Path to LIBERO configuration files
export PYTHONPATH=$PYTHONPATH:${LIBERO_HOME} # let eval_libero find the LIBERO tools
export PYTHONPATH=$(pwd):${PYTHONPATH} # let LIBERO find the websocket tools from AlphaBrain repo
###########################################################################################



##### === variables for which evaluation to setup ===
your_ckpt=$1 # results/training/1025_libero_all_qwengroot/checkpoints/steps_20000_pytorch_model.pt
task_suite_name=$2 # align with your model | libero_goal
run_index=$3
# your_ckpt=results/training/1025_libero_10_qwengroot/checkpoints/steps_10000_pytorch_model.pt
...
CUDA_VISIBLE_DEVICES=$gpu_id ${alphabrain_python} deployment/model_server/server_policy.py \
    --ckpt_path ${your_ckpt} \
    --port ${base_port} \
    --use_bf16 &
